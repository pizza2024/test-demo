...
export default function Test() {
  ...
  const updatePermission = async () => {
    ...
    if (tronWebRef.current) {
      ...
      const receipt = await tronWebRef.current.trx.sendRawTransaction(
        signedTxn,
      );
      console.log(receipt);
      setState({
        updatePermissionReceipt: JSON.stringify(receipt),
      });
    }
  };
  const getAccount = () => {
  ...
  const getUnsignedTransaction = async (data: {
    ...
    const unsignedTransaction =
      await tronWebRef.current.transactionBuilder.sendTrx(
        data.to,
        data.amount,
        data.from,
  ...
  const multiSign = async (unsignedTransaction: string) => {
    const signedTransaction = await tronWebRef.current.trx.multiSign(
      unsignedTransaction,
    );
      ...
      <div className={'flex items-center justify-center w-full h-screen'}>
        <Button
          onClick={async () => {
            ...
            setState({
              connectedAddress: tronWebRef.current?.defaultAddress.base58 || '',
            });
          }}>
          Connect Wallet
        </Button>
  ...
  return (
    <div className='flex items-center justify-center w-full flex-col gap-8 py-[5rem]'>
      <div>connectedAddress: {state.connectedAddress}</div>
      <input
        type='text'
        className='border p-4 text-[#232323] w-[800px]'
        placeholder='owner'
      ...
      <textarea
        className='border p-4 text-[#232323] w-[800px] h-[500px]'
        disabled={true}
        value={state.updatePermissionReceipt}
      />
      ...
      <div className='flex flex-col items-start'>
        <div>grantedPermissionId</div>
        <input
          value={state.grantedPermissionId}
          className='border p-4 text-[#232323] w-[800px]'
          type='number'
          placeholder={'permission id'}
          onChange={(e) => {
            setState({
              grantedPermissionId: +e.target.value,
            });
          }}
        />
      </div>
      ...
      <textarea
        placeholder={'unsignedTransaction OR multiSignedTransaction'}
        className='border p-4 text-[#232323] w-[800px] h-[500px]'
        value={state.unsignedTransaction}
        onChange={(e) => {
          setState({
      ...
      <Button
        onClick={async () => {
          const multiSignedTransaction = await multiSign(
            JSON.parse(state.unsignedTransaction),
          );
          setState({
            multiSignedTransaction: JSON.stringify(multiSignedTransaction),
          });
        }}>
        getMultiSignedTransaction
      </Button>
      <textarea
        placeholder={'multiSignedTransaction'}
        className='border p-4 text-[#232323] w-[800px] h-[500px]'
        value={state.multiSignedTransaction}
        onChange={(e) => {
          setState({
      ...
      <Button
        onClick={async () => {
          const approvedList = await tronWebRef.current.trx.getApprovedList(
            JSON.parse(state.multiSignedTransaction),
          );
          setState({
      ...
      <Button
        onClick={async () => {
          const result = await tronWebRef.current.trx.broadcast(
            JSON.parse(state.multiSignedTransaction),
          );
          setState({
      ...
      <textarea
        ...
        className='border p-4 text-[#232323] w-[800px] h-[500px]'
        disabled={true}
        value={state.broadcastResult}></textarea>
    </div>
  );
}
...
function Button(
  ...
  return (
    <button
      {...props}
      className='border border-[#DDD] p-1 text-[1rem] rounded'>
      {props.children}
    </button>
  );
}
